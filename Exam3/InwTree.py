...
class binary_search_tree:
    ...
    def print_levelorder(self):
        ...
        for i in range(1, h + 1):
            self._print_levelorder(self.root, i)
        print();

    def _print_levelorder(self, root, level):
        if root is None:
            return
        if level == 1:
            print(root.value, end = ' ')
        elif level > 1:
            self._print_levelorder(root.left_child, level - 1)
            self._print_levelorder(root.right_child, level - 1)
    ...
    def _printSideway(self, root, level):
        if root:
            bst._printSideway(root.right_child, level + 1)
            print(' ' * level * 3, root.value, sep='')
            bst._printSideway(root.left_child, level + 1)
